...
export async function persistUserDataJob(
  ...
  try {
    ...
    const channel = await withAutoRefresh(() => yt.getMyChannel());
    const youtubePlaylists = await withAutoRefresh(() => yt.listMyPlaylists());

    // Fetch liked videos first so we can set an accurate trackCount.
    const likesSourcePlaylistId = channel.likesPlaylistId;
    const likedVideoIds = likesSourcePlaylistId
      ? await withAutoRefresh(() => yt.listPlaylistVideoIds(likesSourcePlaylistId))
      : [];
    ...
    const playlistsDomain = [
      ...youtubePlaylists.map((p) => ({
        ...
        images: [],
        externalUrls: { spotify: `https://www.youtube.com/playlist?list=${p.id}` },
        snapshotId: "",
      })),
      {
        id: "liked-songs",
        name: "Liked Songs",
        description: "Your liked videos from YouTube Music",
        ownerId: userId,
        public: false,
        collaborative: false,
        trackCount: likedVideoIds.length,
        images: [],
        externalUrls: { spotify: "https://music.youtube.com/playlist?list=LM" },
        snapshotId: "",
      },
    ];
    ...
    const storePlaylistVideos = async (playlistId: string, videoIds: string[]) => {
      const videos = await withAutoRefresh(() => yt.getVideosByIds(videoIds));

      const tracks: Track[] = videos.map((v) => {
        uniqueArtists.set(v.channelId, { id: v.channelId, name: v.channelTitle });
        return {
          id: v.id,
          name: v.title,
          ...
          album: {
            ...
            images: [],
            externalUrls: { spotify: `https://www.youtube.com/channel/${v.channelId}` },
          },
        };
      });
    ...
    // Store liked videos as "liked-songs"
    if (likedVideoIds.length > 0) {
      await storePlaylistVideos("liked-songs", likedVideoIds);
    }
